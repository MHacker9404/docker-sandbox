...
app.post('/signup', async (req, res) => {
  ...
  try {
    const hashedPW = await axios.get(`http://${process.env.AUTH_DOMAIN}:5000/hashed-password/` + password);
...
app.post('/login', async (req, res) => {
  ...
  const response = await axios.get( `http://${process.env.AUTH_DOMAIN}:5000/token/` + hashedPassword + '/' + password);
